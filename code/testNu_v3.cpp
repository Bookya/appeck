...
// DEFINE TRACE FOLDER LOCATION
string trace_folder_location="/var/www/appeck/Beta/upload/";
// END DEFINE
...
int prop_tocheck_xml(string xml_line, string prop_name, string prop_type, bool prop_value){
    ...
    size_t left_border, right_border, found_rec,
    ...
    string temp_str1 = prop_name.substr(0, found)+'='+prop_name.substr(found+1, found2-found-1),
           temp_str2,
           temp_str3;
    //cout<<temp_str1<<endl;
    found = xml_line.find(temp_str1);
    found_rec = found;//0606
    ...
    //if (found==string::npos && prop_type=="exists")
        ...
        left_border = xml_line.rfind("<node", found, 5); //cout<<left_border<<endl;
        right_border = xml_line.find(">",found, 1); //cout<<right_border<<endl;
        found = found2;
        //if contents have _ ? -> then need change divid word '_'
        found2= prop_name.find('_', found+1);
        temp_str2 = prop_name.substr(found+1, found2-found-1)+"=\""+prop_name.substr(found2+1, prop_name.size()-found2)+"\"";
        //cout<<" "<<temp_str2<<endl;
        found = xml_line.find(temp_str2, left_border);
        //cout<<left_border<<'_'<<found<<'_'<<right_border<<endl;
//cout<<xml_line.substr(left_border, right_border-left_border+1)<<endl;
        //0117 change order of if & else if
        if (found!=string::npos && prop_type=="exists" && found<right_border) return prop_value;
        else if (found==string::npos) return 0;
        else if (found>right_border) {
            while (found!=string::npos) {
                //if (c_index==0) {
                    left_border = xml_line.rfind("<node", found, 5);
                    right_border = xml_line.find(">",found, 1);
                //}
                found = xml_line.find(temp_str1, left_border);
                if (found!=string::npos && found<right_border) c_index=1;
                found = xml_line.find(temp_str2, right_border); //cout<<found<<endl;
            }
            if (c_index==0) return 0;
        }

        if (prop_value) temp_str3 = prop_type+"=\"true\"";
        else temp_str3 = prop_type+"=\"false\"";
        temp_str3 = " "+temp_str3+" ";
        //cout<<"  "<<temp_str3<<endl;
        found = xml_line.find(temp_str3, left_border);
...
string get_xml_path(string user_app, string state_express ){
    ...
    string xml_path = trace_folder_location+user_app+"/";
...
string get_transNum_path(string automata_dir, string trans_line) {
    ...
    return trace_folder_location+user_app+"/"+temp_str.substr(0, found)
